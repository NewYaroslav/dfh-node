...
UnifiedGate::UnifiedGate(AuthService &auth_service, RateLimiter &rate_limiter,
                         WsConnectionLimiter &ws_limiter)
    : m_auth_service(auth_service), m_rate_limiter(rate_limiter),
      m_ws_limiter(ws_limiter) {}

GateResult UnifiedGate::authorize_http(const std::string &token,
                                       TaskKind kind) {
    ...
    if (!m_rate_limiter.check_and_record(context.fingerprint,
                                         context.rps_limit)) {
...
GateResult UnifiedGate::authorize_ws_upgrade(const std::string &token) {
    ...
    if (!m_ws_limiter.register_connection(context.fingerprint,
                                          context.ws_max_connections)) {
        return GateError{GateErrorCode::ConnectionLimited,
...
GateResult UnifiedGate::authorize_ws_message(const std::string &fingerprint,
                                             TaskKind kind) {
