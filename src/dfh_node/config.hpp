...
struct QueuesConfig {
    std::int64_t high_capacity =
        10000; ///< Ёмкость high-priority очереди, элементов.
    std::int64_t low_capacity =
        5000;        ///< Ёмкость low-priority очереди, элементов.
...
struct ApiKeyEntry {
    ...
    std::int64_t ws_max_connections =
        10; ///< Лимит одновременных WS-соединений.
};
...
struct AuthConfig {
    ...
    std::int64_t rate_limit_window_ms =
        1000; ///< Окно rate-limit в миллисекундах.
    std::vector<ApiKeyEntry>
        api_keys{}; ///< Ключи в виде fingerprint без plaintext.
};
...
struct StorageConfig {
    ...
    std::int64_t min_free_bytes =
        2000000000; ///< Минимум свободного места, байты.
};
...
struct LoggingConfig {
    std::string level =
        "info";          ///< Уровень логирования (trace|debug|info|warn|error).
