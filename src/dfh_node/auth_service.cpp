 ...
 * \details Сначала выполняет lookup в cache/store, затем проверяет срок действия и scope.
 */
...
GateResult AuthService::lookup_and_validate(const std::string& fingerprint) {
    ...
    const AuthContext context{
        fingerprint, record->scope_mask, record->rps_limit, record->ws_max_connections, record->expires_at_ms};
