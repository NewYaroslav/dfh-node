...
void log_message(logit::LogLevel level, const char *file, int line,
...
void init_logging(const config::LoggingConfig &log_cfg) {
    ...
    if (!log_cfg.file_path.empty()) {
        try {
            ...
            if (log_cfg.console) {
                log_warn(__FILE__, __LINE__, DFH_LOG_FUNCTION,
                         "Failed to init file logger: %s", ex.what());
            } else {
...
void log_trace(const char *file, int line, const char *function,
               const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    log_message(logit::LogLevel::LOG_LVL_TRACE, file, line, function, fmt,
                args);
    va_end(args);
}
...
void log_debug(const char *file, int line, const char *function,
    ...
    va_list args;
    va_start(args, fmt);
    log_message(logit::LogLevel::LOG_LVL_DEBUG, file, line, function, fmt,
                args);
    va_end(args);
}
...
void log_error(const char *file, int line, const char *function,
    ...
    va_list args;
    va_start(args, fmt);
    log_message(logit::LogLevel::LOG_LVL_ERROR, file, line, function, fmt,
                args);
    va_end(args);
}
